...
const cityCoords = c => CITIES[c.trim().toLowerCase()] || null
...
/* ── Extract nakshatra safely (may be string or { name } object) ───────────── */
const nkStr = n => !n ? '' : typeof n === 'string' ? n : (n.name || n.nakshatra_name || n.nak || JSON.stringify(n))

/* ── Extract compact summary ───────────────────────────────────────────────── */
function extractSummary(planets, dob, tob, city) {
  const find = name => planets.find(p => p.name === name)
  const asc = find('Ascendant'), sun = find('Sun'), moon = find('Moon')
  const rows = []
  if (asc)  rows.push(`Ascendant (Lagna): ${asc.sign}`)
  if (sun)  rows.push(`Sun Sign: ${sun.sign} — House ${sun.house}, Nakshatra: ${nkStr(sun.nakshatra)}`)
  if (moon) rows.push(`Moon Sign: ${moon.sign} — House ${moon.house}, Nakshatra: ${nkStr(moon.nakshatra)}`)
  rows.push('')
  ;['Mars','Mercury','Jupiter','Venus','Saturn','Rahu','Ketu'].forEach(name => {
    const p = find(name)
    if (p) rows.push(`${name}: ${p.sign}, House ${p.house}${p.retrograde ? ' (Retrograde)' : ''}`)
  })
  const ret = planets.filter(p => p.retrograde && !['Ascendant'].includes(p.name))
  if (ret.length) rows.push(`\nRetrograde: ${ret.map(p=>p.name).join(', ')}`)
  rows.push(`\nDOB: ${dob}  Time: ${tob}  Place: ${city}`)
  return rows.join('\n')
}

/* ── Format chart table ────────────────────────────────────────────────────── */
const P_EMOJI = { Sun:'☀️',Moon:'🌙',Mars:'♂️',Mercury:'☿',Jupiter:'♃',Venus:'♀️',Saturn:'♄',Rahu:'☊',Ketu:'☋',Ascendant:'⬆️' }
function formatChart(planets, dob, tob, city) {
...
/* ── Gemini interpretation (summary + question only, no history) ───────────── */
...
Use the birth chart below to answer the question precisely.
Do NOT use generic horoscope language or phrases like "the universe says".
Focus on: emotional patterns, career tendencies, relationship behavior, strengths, and specific planetary influences.
...
Answer in 2–3 focused, conversational paragraphs. Be specific and insightful.` }] }],
      generationConfig: { temperature:0.82, maxOutputTokens:600 },
...
/* ── Reading count (localStorage, daily reset) ─────────────────────────────── */
function getCount() {
...
/* ── Keyword detectors ─────────────────────────────────────────────────────── */
const KUNDLI_KW = ['kundli','kundali','birth chart','janam','natal chart','generate','my chart','create kundli']
const isKundliReq = t => KUNDLI_KW.some(k => t.toLowerCase().includes(k))
...
const BUBBLE_MSGS = [
  ...
  '💫 Ask about career, love, or Dasha',
  '☽ Discover your Moon & Ascendant sign',
]
...
function AttentionBubble({ msg, onOpen, onClose }) {
  return (
    <div style={{position:'absolute',bottom:66,right:0,animation:'bubbleIn 0.4s cubic-bezier(.22,1,.36,1)',zIndex:1}}>
      <div onClick={onOpen} style={{background:'linear-gradient(135deg,#2a1f12,#1e1810)',border:'1px solid rgba(201,169,110,0.4)',borderRadius:'14px 14px 4px 14px',padding:'10px 14px 10px 12px',cursor:'pointer',boxShadow:'0 8px 28px rgba(0,0,0,0.45)',whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:8,transition:'border-color 0.2s'}} onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(201,169,110,0.7)'} onMouseLeave={e=>e.currentTarget.style.borderColor='rgba(201,169,110,0.4)'}>
        <span style={{fontFamily:"'Glacial Indifference',sans-serif",fontSize:12.5,color:'#f5ede0'}}>{msg}</span>
        <button onClick={e=>{e.stopPropagation();onClose()}} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.3)',fontSize:14,lineHeight:1,padding:'0 0 0 4px',transition:'color 0.2s',flexShrink:0}} onMouseEnter={e=>e.currentTarget.style.color='#fff'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,0.3)'}>×</button>
      </div>
...
/* ── Suggestion chips ──────────────────────────────────────────────────────── */
const CHIPS = [
  { label:'🔯 Generate my Kundli',          kundli:true },
  { label:'💼 Career & life purpose',        q:'What does my chart say about my career and life purpose?' },
  { label:'💍 Love & marriage timing',       q:'What does my chart reveal about my love life and marriage?' },
  { label:'🪐 Current Dasha & its effects',  q:'What is my current Mahadasha and how is it affecting my life?' },
  { label:'📅 Book expert consultation',     booking:true },
]
...
export default function AstroChat() {
  ...
  const [msgs,         setMsgs]         = useState([{ role:'bot', text:'Namaste 🙏 I\'m Vedic Saar AI.\n\nI can generate your Janma Kundli and give you deep chart readings — career, love, Dasha, spirituality.\n\nWhat would you like to explore?' }])
  ...
  // Conversational kundli collection state
  const [kundliStep,   setKundliStep]   = useState(null) // null | 'dob' | 'tob' | 'city'
  const [kundliBuf,    setKundliBuf]    = useState({})   // { dob, tob }
  ...
  async function generateKundli(dob, tob, city, coords) {
    setLoading(true)
    try {
      ...
      const summary = extractSummary(planets, dobStr, tobStr, city)
      setAstroSummary(summary)
      setMsgs(m => [...m, { role:'bot', text:formatChart(planets, dobStr, tobStr, city), mono:true }])
    } catch(e) {
  ...
  async function send(overrideText) {
    ...
    /* ── Kundli conversation steps ── */
    if (kundliStep === 'dob') {
      const dob = parseDob(text)
      if (!dob) { setMsgs(m=>[...m,{role:'bot',text:"I couldn't read that date. Try formats like: 26 April 1995, 26/4/1995, or 1995-04-26"}]); return }
      setKundliBuf({ dob })
      setKundliStep('tob')
      setMsgs(m=>[...m,{role:'bot',text:`Got it — ${dob.day}/${dob.month}/${dob.year} ✓\n\nWhat time were you born? (e.g. 10:30 AM or 22:45)`}])
      return
    }
    if (kundliStep === 'tob') {
      const tob = parseTob(text)
      if (!tob) { setMsgs(m=>[...m,{role:'bot',text:"I couldn't read that time. Try formats like: 10:30 AM, 1:41 PM, or 22:45"}]); return }
      setKundliBuf(b => ({ ...b, tob }))
      setKundliStep('city')
      setMsgs(m=>[...m,{role:'bot',text:`${String(tob.hour).padStart(2,'0')}:${String(tob.minute).padStart(2,'0')} ✓\n\nWhich city were you born in?`}])
      return
    }
    if (kundliStep === 'city') {
      const coords = cityCoords(text)
      if (!coords) { setMsgs(m=>[...m,{role:'bot',text:`I don't have coordinates for "${text}". Try a major Indian city like Delhi, Mumbai, Jaipur, or Bangalore.`}]); return }
      setKundliStep(null)
      await generateKundli(kundliBuf.dob, kundliBuf.tob, text, coords)
      return
    }
    ...
    if (isKundliReq(text)) {
      setKundliStep('dob')
      setMsgs(m=>[...m,{role:'bot',text:"Sure! Let's generate your Kundli. 🔯\n\nWhat's your date of birth? (e.g. 26 April 1995 or 26/4/1995)"}])
      return
    }

    /* ── Chart not yet generated ── */
    if (!astroSummary) {
      setMsgs(m=>[...m,{role:'bot',text:'I need your birth chart first to give you accurate insights.\n\nType "generate my kundli" or tap the chip below to get started. 🔯'}])
      return
    }
    ...
    try {
      ...
      const footer = left > 0
        ...
        : `\n\n— Free readings used up. Book a session for unlimited insights. —`
      ...
      {open && (
        <div className="chat-panel" style={{position:'fixed',bottom:90,right:24,zIndex:9999,width:'min(400px,calc(100vw - 32px))',height:'min(640px,calc(100vh - 110px))',background:panel,border:'1px solid rgba(201,169,110,0.22)',borderRadius:16,boxShadow:'0 28px 80px rgba(0,0,0,0.6)',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:"'Glacial Indifference',sans-serif"}}>
          ...
          <div style={{padding:'12px 16px',flexShrink:0,borderBottom:'1px solid rgba(201,169,110,0.12)',background:'linear-gradient(90deg,#201a12,#1a1410)',display:'flex',alignItems:'center',gap:10}}>
            ...
            <button onClick={()=>setOpen(false)} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.3)',fontSize:22,lineHeight:1,padding:'2px 6px',transition:'color 0.2s',flexShrink:0}} onMouseEnter={e=>e.currentTarget.style.color='#fff'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,0.3)'}>×</button>
          </div>
          ...
          <div className="chat-scroll" style={{flex:1,overflowY:'auto',padding:'12px 12px 6px',display:'flex',flexDirection:'column',gap:10,scrollbarWidth:'thin',scrollbarColor:'rgba(201,169,110,0.2) transparent'}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:6,marginTop:2}}>
                {CHIPS.map((s,i)=>(
                  <button key={i} className="chat-chip"
                    onClick={()=>{ if(s.kundli) send('generate my kundli'); else if(s.booking) handleBook(); else send(s.q) }}
            ...
            {/* Chips after chart: show after every bot response while chart is available */}
            {astroSummary && !loading && msgs[msgs.length-1]?.role === 'bot' && !msgs[msgs.length-1]?.blurred && (
              <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:2}}>
                {[
                  {label:'💼 Career',        q:'What does my chart say about my career?'},
                  {label:'💍 Love & marriage',q:'What does my chart say about love and marriage?'},
                  {label:'🪐 Current Dasha',  q:'What is my current Mahadasha and its effects?'},
                  {label:'🧘 Spiritual path', q:'What does my chart reveal about my spiritual path?'},
                  {label:'💎 Lucky gemstone', q:'Which gemstone is most beneficial for me based on my chart?'},
                ].map((c,i)=>(
            ...
            {/* Prompt chip when no chart yet and conversation has started */}
            {!astroSummary && !loading && msgs.length > 1 && kundliStep === null && msgs[msgs.length-1]?.role === 'bot' && (
              <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:2}}>
                {[
                  {label:'🔯 Generate my Kundli', action:'kundli'},
                  {label:'📅 Book expert consultation', action:'book'},
                ].map((c,i)=>(
                  <button key={i} className="chat-chip"
                    onClick={()=>{ if(c.action==='kundli') send('generate my kundli'); else handleBook() }}
                    style={{background:'rgba(201,169,110,0.06)',border:'1px solid rgba(201,169,110,0.18)',borderRadius:20,padding:'6px 12px',cursor:'pointer',fontFamily:"'Glacial Indifference',sans-serif",fontSize:11.5,color:'rgba(201,169,110,0.85)',transition:'background 0.18s,border-color 0.18s'}}
                  >{c.label}</button>
                ))}
              </div>
          ...
          <div style={{padding:'8px 10px',borderTop:'1px solid rgba(201,169,110,0.1)',display:'flex',gap:8,alignItems:'flex-end',flexShrink:0,background:'rgba(0,0,0,0.18)'}}>
            <textarea ref={inputRef} className="chat-input" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey}
              placeholder={kundliStep==='dob'?'e.g. 26 April 1995 or 26/4/1995':kundliStep==='tob'?'e.g. 10:30 AM or 22:45':kundliStep==='city'?'e.g. Delhi, Mumbai, Jaipur…':astroSummary?'Ask anything about your chart…':'Ask about your Kundli or astrology…'}
              rows={1}
